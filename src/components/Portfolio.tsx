...
export default function Box(): JSX.Element {
  ...
  const images = {
    tree: {
      src: '/дерево.png',
      description: 'Работа выполнена для сайта, освещающего экологические проблемы Карелии. Она передает чувство тревоги и печали через изображение дерева за разбитым стеклом, символизирующее разрушение природы. Атмосфера мрачная и безнадёжная, подчеркивающая необходимость защиты и восстановления природных ресурсов региона.'
    },
    jungle: {
      src: '/джунгли.png',
      description: 'Работа выполнена для сайта, организующего туристические походы через джунгли. Изображение передает ощущение густых тропических лесов, полное погружения в дикую природу. Визуализация создает атмосферу таинственности и приключений, приглашая зрителя стать частью этого захватывающего мира.'
    },
    ball: {
      src: '/шарик.png',
