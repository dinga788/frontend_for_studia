...
const KatalogPage = () => {
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      const workerResponse = await ordersAPI.getWorker(1);
      if (!workerResponse.data) {
        addNotification('Работник не найден', 'error');
        return;
      }
  
      //Получаем цену услуги
      const priceString = services[currentService].price
      .replace(/\s/g, '')
      .replace('р.', '');
      ...
      const orderData = {
        ...
        serviceId: services[currentService].id,
        workerId: 1,
        deadline: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString(),
        appointmentDate: new Date().toISOString(),
        status: "В работе",
        paymentStatus: "Оплачен",
        total_price: parseFloat(priceString)
      };
  ...
  return (
    <div className="w-[1921px] h-[1080px]">
      ...
      <Dno className="fixed bottom-0 left-0 w-full z-10"/> 
      {showAuthModal && (
